...
* APP\_NAME => `(terserah)`,
* APP\_ENV => production,
* APP\_KEY => `(ambil dari php artisan key:generate)`,
* APP\_DEBUG => false,
* APP\_TIMEZONE => `(sesuaikan)`
...
* APP\_URL => `(paste aja Deployment URLnya)`
* ASSET\_URL => `(paste aja Deployment URLnya)`
...
* Untuk file JSON statis (misal: `mahasiswa.json`, `galleries.json`), simpan saja di `database/data/` atau `storage/app/`.
* Gunakan controller untuk load JSON dan parsing datanya ke blade jika ingin dynamic.
* Tambahkan `node_modules/` ke dalam `.gitignore` agar tidak ikut di-commit:
...
Mau bantu improve? Pull request terbuka untuk:

* Menambahkan dokumentasi
* Memperbaiki bugs
* Menambahkan fitur seperti DB eksternal, authentication, dll
